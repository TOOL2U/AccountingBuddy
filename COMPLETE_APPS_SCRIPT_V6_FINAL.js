 ...
 * Version 7.0: Added Bank & Cash Balance Management
 *
 ...
 * ✨ Balance Management: Track bank & cash balances
 * ✨ Balance Append: { action: "balancesAppend", secret: "...", bankBalance: 1000, cashBalance: 500 }
 * ✨ Balance Get: { action: "balancesGetLatest", secret: "..." }
 * ✨ P&L Reconciliation: Returns Month/Year Net Cash for variance calculation
 * ✨ Automatic balance carry-forward if not provided
 *
 * ALL FEATURES:
 ...
 * ✨ Balance Management: { action: "balancesAppend" | "balancesGetLatest", secret: "..." }
 ...
 * - Balance Append: POST with { action: "balancesAppend", secret: "...", bankBalance?: number, cashBalance?: number, note?: string }
 * - Balance Get: POST with { action: "balancesGetLatest", secret: "..." }
 */
...
function doPost(e) {
  try {
    ...
    if (payload.action === 'getPnL') {
      ...
      return createErrorResponse('Unknown request type. Expected: getPnL, getInbox, deleteEntry, balancesAppend, balancesGetLatest, list_named_ranges, or webhook data');
    }
 ...
 * Appends a new balance entry to the "Bank & Cash Balance" sheet
 * Carries forward previous values if not provided
 *
 * @param {Object} payload - { bankBalance?: number, cashBalance?: number, note?: string }
 * @returns {ContentService.TextOutput} JSON response
 */
function handleBalancesAppend(payload) {
  try {
    Logger.log('=== handleBalancesAppend ===');
    ...
    if (!balancesSheet) {
      ...
      return createErrorResponse('Balances sheet not found. Please create a "' + BALANCES_SHEET_NAME + '" tab with columns: timestamp, bankBalance, cashBalance, note');
    }
    ...
    if (lastRow > 1) {
      ...
      Logger.log('Previous balances - Bank: ' + latestBankBalance + ', Cash: ' + latestCashBalance);
    }
    ...
    Logger.log('New balances - Bank: ' + bankBalance + ', Cash: ' + cashBalance);
...
/**
 * Handle balance get latest request
 * Returns the latest balance entry and reconciliation data from P&L
 *
 * @returns {ContentService.TextOutput} JSON response with latest balance and reconcile data
 */
function handleBalancesGetLatest() {
  try {
    Logger.log('=== handleBalancesGetLatest ===');
    ...
    if (lastRow > 1) {
      ...
      Logger.log('Latest balance - Bank: ' + latest.bankBalance + ', Cash: ' + latest.cashBalance);
    } else {
      Logger.log('No balance entries found, returning defaults');
    }
    ...
    Logger.log('✓ Returning balance data');
